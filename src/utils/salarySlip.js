...
export const getPeriodKey = (year, monthIndex) => `${year}-${`${monthIndex + 1}`.padStart(2, "0")}`;

export const parsePeriodKeyFromMonthLabel = (label) => {
  if (!label) return "";
  const parsed = new Date(`1 ${label}`);
  if (Number.isNaN(parsed.getTime())) return "";
  return getPeriodKey(parsed.getFullYear(), parsed.getMonth());
};

export const getPeriodKeyFromSlip = (slip) => {
  if (slip?.period?.key) return slip.period.key;
  if (slip?.period?.year >= 0 && slip?.period?.monthIndex >= 0) {
    return getPeriodKey(slip.period.year, slip.period.monthIndex);
  }
  return parsePeriodKeyFromMonthLabel(slip?.month);
};

export const getPreviousMonthPeriod = (referenceDate = new Date()) => {
  const year = referenceDate.getMonth() === 0 ? referenceDate.getFullYear() - 1 : referenceDate.getFullYear();
  const monthIndex = referenceDate.getMonth() === 0 ? 11 : referenceDate.getMonth() - 1;
  return {
    year,
    monthIndex,
...
export const getCurrentMonthPeriod = (referenceDate = new Date()) => {
...
export const computeSalarySlipForPeriod = ({
  user,
  attendanceSessions = [],
  leaves = [],
  holidays = [],
  ...
  const payroll = user?.payrollDetails || {};
  const salaryCurrency = payroll.currency || "USD";
  ...
  let grossEarned = basic + hra + conveyance + medical + specialAllowance + otherAllowance;
  if (grossEarned < targetGross) {
    otherAllowance += targetGross - grossEarned;
    grossEarned = targetGross;
  }

  const pf = payroll.pfNumber ? Math.round(basic * salaryTemplate.pfRate) : 0;
  const esi = payroll.esiNumber && grossEarned <= 21000 ? Math.round(grossEarned * salaryTemplate.esiRate) : 0;
  const professionalTax = grossEarned > 0 ? salaryTemplate.professionalTax : 0;
  ...
  const net = Math.max(grossEarned - totalDeductions, 0);
